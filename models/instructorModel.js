...
class InstructorModel {
  async getInstructorById(instructorId) {
    ...
    try {
      ...
      const instructorQuery = await connection.execute(
        `SELECT * FROM persons
         WHERE
          persons.person_id = ?` , [instructorId]
      );

      const socialQuery = await connection.execute(
        `SELECT * FROM person_socials
        LEFT JOIN 
            social_icons ON social_icons.icon_id = person_socials.social_id
        WHERE
            person_socials.person_id = ?` , [instructorId]
      );
      
      const classesQuery = await connection.execute(
        `SELECT * FROM class_instructors
        LEFT JOIN 
            academy_classes ON class_instructors.class_id = academy_classes.class_id
        LEFT JOIN 
            class_terms ON class_terms.term_id = academy_classes.class_term
        WHERE
            class_instructors.person_id = ? 
            AND academy_classes.class_status = 'LIVE'`
         , [instructorId]
      );

      const [rows1, rows2, rows3] = await Promise.all([instructorQuery, socialQuery, classesQuery]);
        // console.log(`Rows1: ${rows1[0]}`);
        // console.log(`Rows2: ${rows2[0]}`);
        // console.log(`Rows3: ${rows3[0]}`);
      connection.release();
      return { persons: rows1[0], social_icons: rows2[0], classes: rows3[0] };
    } catch (error) {
      throw error;
    }
  }
