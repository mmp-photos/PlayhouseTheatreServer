...
export async function getUsers(req, res) {
    ...
    const idCheck = `SELECT * FROM users WHERE EXISTS (SELECT * from users where id = '${id}')`
    let idExists = await pool.query(idCheck)
    console.log(idExists);
    if(!idExists[0].length && !emailExists[0].length){
        const sql = `INSERT INTO users (id, created_on, first_name, last_name, email, password, user_type) VALUES ('${id}', '${created_on}', '${first_name}', '${last_name}', '${email}', '${hashedPwd}', '${user_type}')`;
